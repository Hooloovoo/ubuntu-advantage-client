...
@when("I change contract key `{key}` to use value `{value}`")
...
@when(
    "I create the contract config overrides file for `{service_list}` using the `{machine}` machine"  # noqa
)
...
@when("I configure the ua-airgapped service")
def i_configure_the_ua_airgapped_service(context):
...
@when("I start the ua-airgapped service")
def i_start_the_ua_airgapped_service(context):

...
Feature: Performing attach using ua-airgapped
    ...
    Scenario Outline: Attached enable Common Criteria service in an ubuntu lxd container
        ...
        And I run `add-apt-repository ppa:yellow/ua-airgapped -y` with sudo
        And I run `apt-get update` `with sudo` on the `mirror` machine
        And I run `apt-get update` with sudo
        And I run `apt-get install apt-mirror get-resource-tokens -yq` `with sudo` on the `mirror` machine
        And I run `apt-get install contracts-airgapped ua-airgapped -yq` with sudo
        ...
        And I run `apt-key adv --keyserver keyserver.ubuntu.com --recv-keys 4067E40313CB4B13` with sudo
        And I create the contract config overrides file for `esm-infra,esm-apps` using the `mirror` machine
        And I configure the ua-airgapped service
        And I start the ua-airgapped service
        And I change contract key `contract_url` to use value `http:\/\/127.0.0.1:8484`
